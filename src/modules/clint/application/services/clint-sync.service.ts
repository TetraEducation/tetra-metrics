...
import { pickEmail, pickName, pickPhone, pickTagKeys } from '@/modules/clint/application/mappers/clint.mapper';
import { chooseBetterName, normalizeName, removeNameDuplication } from '@/modules/clint/application/utils/name-validator';
...
export class ClintSyncService {
  ...
  async run({ dryRun }: { dryRun: boolean }): Promise<ClintSyncReport> {
    ...
    if (!dryRun) {
      for (const o of origins) {
        const origin = o as {
          ...
          name?: string;
          title?: string;
          stages?: Array<{ id?: string; label?: string; order?: number; type?: string }>;
        };
        ...
        if (existingFunnel.error) {
          this.logger.warn(`Erro ao buscar funnel existente para origin ${originId}: ${existingFunnel.error.message}`);
        }
        ...
        if (funnelUp.error) {
          this.logger.error(`Erro ao criar/atualizar funnel para origin ${originId}: ${funnelUp.error.message}`);
          continue;
        }
        ...
        const aliasResult = await this.supabase
          .from('funnel_aliases')
          .upsert(
            { funnel_id: funnelId, source_system: 'clint', source_key: originId },
            { onConflict: 'source_system,source_key' },
          );

        if (aliasResult.error) {
          this.logger.error(`Erro ao criar funnel_alias para origin ${originId}: ${aliasResult.error.message}`);
        } else {
          this.logger.debug(`Funnel criado/atualizado: ${originName} (origin_id: ${originId}, funnel_id: ${funnelId})`);
        }
        ...
        for (const s of stages) {
          ...
          const stageUpsert = await this.supabase
            .from('funnel_stages')
            .upsert(
              { funnel_id: funnelId, key: stageKey, name: stageName, position: pos },
    ...
    this.logger.log('🔵 [CONTACTS] Buscando e processando contatos da API do Clint (página por página)...');
    ...
    while (hasMorePages) {
      // Buscar uma página de contatos
      const pageResult = await this.clintApi.contactsPage(currentPage);
      
      if (currentPage === 1) {
        totalPages = pageResult.totalPages;
        this.logger.log(`🔵 [CONTACTS] Total de páginas: ${totalPages} (~${pageResult.totalCount} contatos)`);
        if (pageResult.data.length > 0) {
          this.logger.log(`🔵 [CONTACTS] Primeiro contato (amostra): ${JSON.stringify(pageResult.data[0], null, 2)}`);
        }
      }
      ...
      if (contacts.length === 0) {
        this.logger.warn(`⚠️ [CONTACTS] Página ${currentPage}/${totalPages} vazia`);
        break;
      }
      ...
      for (let chunkStart = 0; chunkStart < contacts.length; chunkStart += CHUNK_SIZE) {
        ...
        const contactDataMap = new Map<
          string,
          {
            email: string;
            leadId?: string;
            leadData?: { full_name: string; first_contact_at: string; last_activity_at: string };
            identifiers: Array<{ type: string; value: string; value_normalized: string; is_primary: boolean }>;
            source?: { source_ref: string; first_seen_at: string; last_seen_at: string; meta: unknown };
            events: Array<{ event_type: string; occurred_at: string; dedupe_key: string; payload: unknown }>;
            leadTags: Array<{ tag_id: string; source_ref: string; meta: unknown }>;
            tagEvents: Array<{ event_type: string; occurred_at: string; dedupe_key: string; payload: unknown }>;
            leadUpdates?: { last_activity_at?: string; first_contact_at?: string; full_name?: string };
          }
        >();
        ...
        for (const c of chunk) {
          ...
          try {
            await this.processContactForBatch(c, contactNumberGlobal, totalContactsFetched, report, dryRun, contactDataMap);
          } catch (error) {
            this.logger.error(`❌ [CONTACTS] Erro ao processar contato ${contactNumberGlobal}: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
  ...
  private async processContactForBatch(
    ...
    if (!email) {
      ...
      if (contactNumber <= 5) {
        this.logger.warn(`⚠️ [CONTACTS] Contato ${contactNumber} ignorado: sem email. Dados: ${JSON.stringify(c)}`);
      }
      return;
    }
    
    if (contactNumber <= 5) {
      this.logger.log(`🔵 [CONTACTS] Processando contato ${contactNumber}/${totalContacts}: email=${email}`);
    }
    ...
    const contact = c as { id?: string | number; created_at?: string; updated_at?: string };
    const contactCreatedAt = contact?.created_at ? new Date(contact.created_at).toISOString() : null;
    const contactUpdatedAt = contact?.updated_at ? new Date(contact.updated_at).toISOString() : null;
    ...
    if (!leadId) {
      ...
      if (fullName) {
        const currentLead = await this.supabase.from('leads').select('full_name').eq('id', leadId).single();
    ...
    for (const keyRaw of tagKeys) {
      ...
      await this.supabase
        .from('tag_aliases')
        .upsert({ tag_id: tagId, source_system: 'clint', source_key: keyNormalized }, { onConflict: 'source_system,source_key' });
  ...
  private async executeBatchInserts(
    ...
    const newLeads: Array<{ email: string; data: { full_name: string; first_contact_at: string; last_activity_at: string } }> = [];
    ...
    for (const data of contactDataMap.values()) {
      if (data.leadId && data.leadUpdates) {
        updatePromises.push(Promise.resolve(this.supabase.from('leads').update(data.leadUpdates).eq('id', data.leadId)));
      }
    }
    ...
    const identifiersToInsert: Array<{ lead_id: string; type: string; value: string; value_normalized: string; is_primary: boolean }> = [];
    for (const data of contactDataMap.values()) {
      if (data.leadId) {
    ...
    if (identifiersToInsert.length > 0) {
      for (let i = 0; i < identifiersToInsert.length; i += BATCH_INSERT_SIZE) {
        ...
        const result = await this.supabase.from('lead_identifiers').upsert(chunk, { onConflict: 'type,value_normalized' });
        if (result.error) {
    ...
    const sourcesToInsert: Array<{ lead_id: string; source_system: string; source_ref: string; first_seen_at: string; last_seen_at: string; meta: unknown }> = [];
    for (const data of contactDataMap.values()) {
    ...
    if (sourcesToInsert.length > 0) {
      for (let i = 0; i < sourcesToInsert.length; i += BATCH_INSERT_SIZE) {
        ...
        const result = await this.supabase.from('lead_sources').upsert(chunk, { onConflict: 'source_system,source_ref' });
        if (result.error) {
    ...
    const eventsToInsert: Array<{ lead_id: string; event_type: string; source_system: string; occurred_at: string; ingested_at: string; dedupe_key: string; payload: unknown }> = [];
    for (const data of contactDataMap.values()) {
      if (data.leadId) {
    ...
    const leadTagsMap = new Map<string, { lead_id: string; tag_id: string; source_system: string; source_ref: string; meta: unknown }>();
    for (const data of contactDataMap.values()) {
      if (data.leadId) {
    ...
    if (leadTagsToInsert.length > 0) {
      for (let i = 0; i < leadTagsToInsert.length; i += BATCH_INSERT_SIZE) {
        ...
        const result = await this.supabase.from('lead_tags').upsert(chunk, { onConflict: 'lead_id,tag_id,source_system' });
        if (result.error) {
  ...
  private async processContact(
    ...
    const contactDataMap = new Map();
    await this.processContactForBatch(c, contactNumber, totalContacts, report, dryRun, contactDataMap);
  }
